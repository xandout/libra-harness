...
async function executeLc(
  ...
  return new Promise((resolvePromise) => {
    ...
    const slackToolsSystem = [
      ...
      '    cdp ax [max_depth]         - Dump the accessibility tree (roles, labels, inputs). Use this first to understand the page!',
      '    cdp goto <url>             - Navigate to a URL',
      '    cdp click <selector|"text=Label"> - Click an element by CSS selector or accessible text',
      '    cdp type <selector> <text> - Focus element and type text',
      '    cdp press <key>            - Press a key: Enter, Tab, Escape, Backspace, ArrowDown, etc.',
      '    cdp scroll [up|down] [px]  - Scroll page (default: 500px)',
      '    cdp wait-for <selector>    - Wait until element exists in DOM',
      ...
      '',
      'Browser Guidelines:',
      '1. If Chrome is not running, run `start-browser` once. Do not launch raw unmanaged `google-chrome &`.',
      '2. After navigating (`cdp goto <url>`), ALWAYS run `cdp ax` to inspect the accessibility tree. It gives you the exact button/link names and inputs without reading messy HTML.',
      '3. Click elements using `cdp click "text=Button Name"` or standard CSS selectors. Use `cdp press Enter` to submit forms or search boxes after typing.',
      '4. If a site opens links in a new window, use `cdp tabs` and `cdp switch-tab <index>` to target the right tab.',
      '5. For long multi-step tasks, use `slack-post` to send brief updates so the user is informed of progress.',
    ].join('\n');
